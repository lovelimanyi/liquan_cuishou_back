...
public class AdminContextInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		try {
			...
			// 正常状态
			user = (BackUser) request.getSession().getAttribute(
					Constant.BACK_USER);
...
//			logger.warn("===== clientIp 地址:" + clientIp);
//			List<String> orayIps = JedisDataClient.getList("orayIps", 0, -1);
//			logger.warn("===== 花生壳orayIps:" + orayIps);
//			if (clientIp == null || orayIps == null) {
//				request.getSession().removeAttribute(Constant.BACK_USER);
//			}
//			boolean loginFlag = false;
//			for(String oray : orayIps){
//				String ip = InetAddress.getByName(oray).getHostAddress();
//				logger.warn("===== 域名 [" + oray + "] 转成IP: " + ip);
