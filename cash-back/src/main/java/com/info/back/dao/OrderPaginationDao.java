...
//				nameSpace + countSql, map);
		...
		try {
			if(org.apache.commons.lang.StringUtils.isNotBlank(JedisDataClient.get("TOTAL_ORDER_COUNT"))){
				totalRecord = Integer.parseInt(JedisDataClient.get("TOTAL_ORDER_COUNT"));
			}else {
				totalRecord = (Integer) getSqlSessionTemplate().selectOne(nameSpace + countSql, map);
				JedisDataClient.set("TOTAL_ORDER_COUNT",totalRecord.toString(),60*60);
			}
