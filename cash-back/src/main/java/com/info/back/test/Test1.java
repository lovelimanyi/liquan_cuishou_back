...
import com.info.back.controller.TestController;
...
public class Test1 {
    ...
    private static Logger loger = Logger.getLogger(TestController.class);
    @Autowired
    ...
    @Test
    public void test7(){
        orderService.dispatchOrderNew("1545256796","431228199410112072", Constant.SMALL);

    }

    @Test
    public void test8(){
        String payId="154512323";
        if (StringUtils.isNotBlank(payId)) {
            ...
            if (null != repayment &&(!repaymentMoney.equals(repaymentedMoney))) {
                try {
                    ...
                    borrowOrder = dataDao.getAssetBorrowOrder(map);
                    loger.info("sync-borrowOrder:"+borrowOrder);
                    repaymentDetailList = dataDao.getAssetRepaymentDetail(map);
...
//						userContactsList = this.dataDao.getUserContacts(map);
                        ...
                        orderService.dispatchOrderNew(loanId,userInfo.get("idNumber").toString(), Constant.SMALL);
...
//						syncUtils.checkReduction(repayment,localDataDao);
    ...
    public boolean checkLoan(String loanId){
        if(StringUtils.isNotBlank(loanId)){
            HashMap<String,String> map = new HashMap<String,String>();
            map.put("ID", loanId);
            int count = localDataDao.checkLoan(map);
            if(count>0){
                return false;
            }
            return true;
    ...
    public  void saveMmanUserLoan(HashMap<String,Object> borrowOrder,HashMap<String,Object> repaymentMap,Map<String,Object> userInfo){
        ...
        mmanUserLoan.setMerchantNo(String.valueOf(userInfo.get("merchant_number")));
...
//			System.out.println("=====================================================");
            ...
            localDataDao.saveCreditLoanPay(creditLoanPay);
