 ...
 *
 */
public class ThreadPoolDealOverdueLoan extends ThreadGroup {
	private boolean isClosed = false;  
	private LinkedList<Runnable> workQueue;      
	private static int threadPoolID = 1;  
	private static ThreadPoolDealOverdueLoan pool;
	
	private ThreadPoolDealOverdueLoan(int poolSize) {  
		super(threadPoolID + "");      
		setDaemon(true);               
	...
	private class WorkThread extends Thread {
		...
		public WorkThread(int id) {
			...
			super(ThreadPoolDealOverdueLoan.this,"线程"+id);
			this.id =id;
		}
		public void run() {
			while(! isInterrupted()) { 
