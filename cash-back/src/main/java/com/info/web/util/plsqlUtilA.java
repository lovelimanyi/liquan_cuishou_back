...
/**
 * 把数据库取出的字段列（小写）组装成ibatis-sql
 * 
 * @author gaoyuhai
 * 
 */
public class plsqlUtilA {
	public static void main(String[] args) {

		try {
			String str = "borrow_tender_deal.txt";
			tests(str);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void tests(String str) throws Exception {
		int i = 0;
		String privincePath = "E:/mybatis/20160606/" + str;
	...
	public static void excuJdbc(String string, String path, String fileName)
		...
		if (str.length == 1) {
			...
			writeDateToFile(path + "-save1-" + fileName, "#{" + string + "},"
					+ "\r\n");// save
			System.out.println("原=" + string + ">>>>目标=" + str);
		} else {
			...
			writeDateToFile(path + "-save1-" + fileName, "#{" + resultStr
					+ "}," + "\r\n");// save
			System.out.println("原" + string + ">>>>目标=" + resultStr);
		}
