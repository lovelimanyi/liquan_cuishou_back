 ...
 *
 */
public class ThreadPoolWith extends ThreadGroup {
	private boolean isClosed = false;  
	private LinkedList<Runnable> workQueue;      
	private static int threadPoolID = 1;  
	private static ThreadPoolWith pool;
	
	private ThreadPoolWith(int poolSize) {  
		super(threadPoolID + "");      
		setDaemon(true);               
	...
	public static ThreadPoolWith getInstance()
	{
		if(null == pool)
			pool = new ThreadPoolWith(2);//初始化线程池
		return pool;
	}
	...
	private class WorkThread extends Thread {
		...
		public WorkThread(int id) {
			...
			super(ThreadPoolWith.this,"线程"+id);
			this.id =id;
		}
		public void run() {
			while(! isInterrupted()) { 
